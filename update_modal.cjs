...
export default function LandingPage() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate API call to backend table
    await new Promise(resolve => setTimeout(resolve, 1500));
    
    // Here we will connect it to your actual database endpoint
    console.log("Saved to database:", formData);
      ...
      <nav className="sticky top-0 z-50 w-full bg-[#030303]/50 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-4">
            <button onClick={() => setIsModalOpen(true)} className="text-sm font-semibold bg-white text-black px-5 py-2.5 rounded-full hover:scale-105 transition-transform">
              Connect with Sales
            </button>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="flex flex-col items-center">
            ...
            <motion.div variants={fadeIn} className="flex flex-col sm:flex-row items-center justify-center gap-6 w-full">
              <button onClick={() => setIsModalOpen(true)} className="group relative w-full sm:w-auto flex items-center justify-center gap-2 bg-white text-black font-bold px-8 py-4 rounded-full hover:scale-105 transition-all text-lg overflow-hidden">
