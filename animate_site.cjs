...
// 1. Inject the Dashboard Mockup into the Hero Section
               ...
               <div className="h-[400px] md:h-[500px] p-4 flex gap-4">
                  ...
                  <div className="flex-1 flex flex-col gap-4 pt-4">
                     ...
                     <div className="flex-1 bg-black/40 rounded-xl border border-white/5 p-6 flex flex-col justify-end gap-6 overflow-hidden relative">
                       {/* AI analyzing effect */}
                       <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-blue-500 to-transparent opacity-50 animate-[dash_2s_linear_infinite]"></div>
...
// Insert the dashboard mockup just before the closing header tag
page = page.replace(
  /<\/header>/,
  `  ${dashboardMockup}\n      </header>`
);

// 2. Add Scroll Animations (whileInView) to Sections

// Bento Box Features section
page = page.replace(
  /<div className="max-w-7xl mx-auto px-6">/g,
  '<motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7 }} className="max-w-7xl mx-auto px-6">'
);
page = page.replace(
  /<\/div>\s*<\/section>/g,
  '</motion.div>\n      </section>'
...
// We need to fix multiple </div> replacements because of the global replace above, 
// Actually global replace of `</div>\n      </section>` might break if the structure is different.
// Let's do targeted replacements.
// Restore the original file and do it cleaner.
