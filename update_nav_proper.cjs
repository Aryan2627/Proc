...
const parts = page.split('{/* --- ATLAN-STYLE MINIMALIST NAV --- */}');
if (parts.length > 1) {
  const rest = parts[1].split('</nav>');
  rest.shift();
  page = parts[0] + newNav + rest.join('</nav>');
  fs.writeFileSync('src/app/page.tsx', page);
  console.log('Successfully updated nav via split');
}
