...
function removeLogin(filePath) {
  ...
  // Regex to remove Log In buttons across different files (they have slightly different classes)
  // For page.tsx and context layer:
...
// If openFaq state isn't there, we'll inject it.
...
// Insert FAQ right before the Mega Footer
page = page.replace(
  /\{\/\* --- MEGA FOOTER ---\*\/\}/,
  \`\${faqSection}\n      {/* --- MEGA FOOTER --- */}\`
);
