...
const targetStr = '{/* --- PRICING SECTION --- */}';
const parts = page.split(targetStr);
if (parts.length > 1) {
  page = parts[0] + bigWebsiteSections + '\n      ' + targetStr + parts[1];
  fs.writeFileSync('src/app/page.tsx', page);
  console.log('Injected missing middle sections!');
} else {
  console.log('Failed to find PRICING SECTION');
}
