...
// 2. Marquee
const parts = page.split('{/* --- LOGO STRIP --- */}');
if (parts.length > 1) {
  const rest = parts[1].split('</section>');
  rest.shift(); // remove the old section
...
`;
  ...
  page = parts[0] + logoMarquee + rest.join('</section>');
}

// 3. Middle sections
const faqParts = page.split('{/* --- FAQ SECTION --- */}');
...
`;
  ...
  page = faqParts[0] + bigWebsiteSections + '\n      {/* --- FAQ SECTION --- */}' + faqParts[1];
}

// 4. Mega footer
const footerParts = page.split('{/* --- FOOTER --- */}');
if (footerParts.length > 1) {
  const rest = footerParts[1].split('</footer>');
  rest.shift();
...
`;
  page = footerParts[0] + megaFooter + rest.join('</footer>');
}

fs.writeFileSync('src/app/page.tsx', page);
console.log('Injected sections flawlessly.');
