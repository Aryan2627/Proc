...
// 1. Update the button in the main landing page to point to /vip
let mainCode = fs.readFileSync('src/app/page.tsx', 'utf8');
...
// Make sure Link is imported (it usually is, but let's check)
if (!mainCode.includes("import Link from 'next/link'")) {
  mainCode = mainCode.replace("import React", "import React from 'react';\nimport Link from 'next/link';");
}
fs.writeFileSync('src/app/page.tsx', mainCode, 'utf8');

// 2. Create the /vip directory and page
if (!fs.existsSync('src/app/vip')) {
  fs.mkdirSync('src/app/vip');
}
...
`;
fs.writeFileSync('src/app/vip/page.tsx', vipCode, 'utf8');
console.log("VIP page created");
